...
module.exports = {
  ...
  async checkEmail(ctx) {
    ...
    console.log(address);
    const toLower =  address.toLowerCase()
    const profile = await strapi.services.profile.findOne({address_contains: toLower},{user: 1});
    return sanitizeEntity(profile.user, { model: strapi.models.profile });
  },
  ...
  async updateEmail(ctx) {
    ...
    const { id } = ctx.params;
    const entity = await strapi.query('user', 'users-permissions').update({id:id}, {email: email});
